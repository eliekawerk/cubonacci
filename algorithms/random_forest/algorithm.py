...
class Algorithm:
    """
    Every Algorithm class needs to implement at least the `__init__`, `fit` and `predict` methods.
    """

    def __init__(self, criterion, max_depth, max_features):
        """
        The __init__ method gets called when initializing the algorithm. The arguments that are named in the method are
        hyperparameters that influence the way the algorithm is constructed and trained. These can be directly linked to
        the algorithm but also for the preprocessor which is considered to be part of the algorithm. In this example random
    ...
    def fit(self, X, y):
        ...
        self.model = RandomForestClassifier(
            n_estimators=100, criterion=self.criterion, max_depth=self.max_depth, max_features=self.max_features
        )
